...
export default async function link(options, shouldRenderSuccess = true) {
    ...
    const remoteAppConfigurationExtension = remoteAppConfigurationExtensionContent(remoteExtensionRegistrations.app.configurationRegistrations, specifications.filter((spec) => spec.appModuleFeatures().includes('app_config')));
    const localAndRemoteApiClientConfiguration = mergeAppConfiguration({ ...localApp.configuration, path: configFilePath }, remoteApp);
        ...
        renderSuccess({
            ...
            nextSteps: [
                ...
                [
                    ...
                    {
                        command: formatPackageManagerCommand(localApp.packageManager, `shopify app ${usingVersionedAppConfig ? 'deploy' : 'config push'}`),
                    },
                ],
            ],
            reference: [
...
async function loadAppConfigFromDefaultToml(options, specifications) {
    try {
...
export function remoteAppConfigurationExtensionContent(configRegistrations, configSpecifications) {
    let remoteAppConfig = {};
