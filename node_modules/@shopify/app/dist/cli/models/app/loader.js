...
class AppLoader {
    ...
    async loadExtensions(appDirectory, appConfiguration) {
    ...
    async createConfigExtensionInstances(directory, appConfiguration) {
        return this.specifications
            .filter((specification) => specification.appModuleFeatures().includes('app_config'))
            .map(async (specification) => {
            const specConfiguration = await parseConfigurationObject(specification.schema, appConfiguration.path, appConfiguration, this.abortOrReport.bind(this));
            const { path, ...specConfigurationWithouPath } = specConfiguration;
            if (Object.keys(specConfigurationWithouPath).length === 0)
                return;
            return this.createExtensionInstance(specification.identifier, specConfiguration, appConfiguration.path, directory).then((extensionInstance) => this.validateConfigurationExtensionInstance(appConfiguration.client_id, extensionInstance));
        });
    }
    async validateConfigurationExtensionInstance(apiKey, extensionInstance) {
